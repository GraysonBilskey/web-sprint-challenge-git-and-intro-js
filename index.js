...
function getArtistByIndex(array, idx) {
  const str = `the artist at index ${idx} is ${array[idx]["name"]}`;
  return str;
}
...
function listOfNames(array) {
  ...
  for (let i = 0; i < namesArray.length; i++) {
    newArray.push(namesArray[i].name);
  }
  return newArray;
}
...
function removeArtist(array, idx) {
  const minusOneArray = [...array];
  minusOneArray.splice(idx, 1);
